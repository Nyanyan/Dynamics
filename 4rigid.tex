...
これは、物体の重心を通る回転軸Gと平行な軸Oを別のところにとった場合、O軸での慣性モーメント$I_O$は$I_G$と、$OG$間の距離$d$、物体の質量$M$によって求まることを意味しています。図で軸は紙面から飛び出ていると考えてください。

\begin{figure}[htbp]
\begin{center}
\begin{tikzpicture}[domain=-0:10,samples=300,>=stealth]
   % 座標軸
   \fill[lightgray] (-0.5,-0.5)--(4,-0.5)--(4,4)--(-0.5,4)--(-0.5,-0.5);
   \draw[->] (-0,0) -- (5,0) node[right] {$x$};
   \draw[->] (0,0) -- (0,5) node[above] {$y$};
   \draw[->] (2,2) -- (5,2) node[right] {$x'$};
   \draw[->] (2,2) -- (2,5) node[above] {$y'$};
   \node [anchor=north] at (0,0) {O};
   \node [anchor=north west] at (2,2) {G};
   
   \fill[black] (3.5,3) circle[radius=0.05] node [anchor=west]{$m_i$};
   
   \draw[dashed](2,2)--(0,2) node [anchor=east]{$y_G$};
   \draw[dashed](2,2)--(2,0) node [anchor=north]{$x_G$};
   \draw[dashed](3.5,3)--(3.5,2) node [anchor=north west]{$x'_i$}--(3.5,0) node [anchor=north]{$x_i$};
   \draw[dashed](3.5,3)--(2,3) node [anchor=south east]{$y'_i$}--(0,3) node [anchor=east]{$y_i$};
   
   \draw[<->](0.1,0.1)--(1.9,1.9) node at (1,0.8) {$d$};
...
\begin{eqnarray}
    I_O&=&\sum_i m_i(x_i^2+y_i^2) \notag \\
    &=&\sum_i m_i((x_G+x'_i)^2+(y_G+y'_i)^2) \notag \\
    &=&\sum_i m_i(x_G^2+y_G^2)+\sum_i m_i(x_i'^2+y_i'^2)+2\sum_i m_ix_G x'_i+2\sum_i m_iy_G y'_i \notag \\
    &=&(x_G^2+y_G^2)\sum_i m_i+\sum_i m_i(x_i'^2+y_i'^2)+2x_G\sum_i m_ix'_i+2y_G\sum_i m_iy'_i \notag \\
    &=&Md^2+I_G \notag
\end{eqnarray}

最後の式変形で、$(x_G^2+y_G^2)=d^2$、$\sum_i m_i=M$です。また、式変形前の右辺第二項は重心を通る軸周りの慣性モーメント$I_G$に等しくなります。問題は右辺第三項、第四項です。結論から言うと$\sum$内が$0$になるのですが、その証明を少ししましょう。
