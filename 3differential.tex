...
\begin{enumerate}
    \item $D\geq 0 \Leftrightarrow \mu \geq \kappa$\\
            ...
            \begin{figure}[!ht]
              \centering
              \includegraphics[width=12cm]{vib_vel1_1.PNG}
            ...
            \begin{tikzpicture}[domain=0:10,samples=300,>=stealth]
               ...
               \draw[line width=0.7pt, domain=0:10] plot (\x,{e^(-0.7/0.8*\x)*cosh(0.8*\x)+1});
               \draw[line width=0.7pt, domain=0:10] plot (\x,{e^(-0.7/0.8*\x)*cosh(0.6*\x)+1});
               \draw[line width=0.7pt, domain=0:10] plot (\x,{e^(-0.7/0.8*\x)*cosh(0*\x)+1});
               \node [anchor=north] at (4,1){$x=e^{-\frac{k'}{2m}t}\left(c_1e^{\gamma't}+c_2e^{-\gamma't}\right)+x_0$};
            
               % 補助線
               \draw [dashed](0,1) node [anchor=east]{$x_0$}--(10,1);
               \draw [dashed](0,2) node [anchor=east]{$c_1+c_2+x_0$};
    ...
    \item $D < 0\Leftrightarrow \mu < \kappa$ \\
            ...
            \begin{tikzpicture}[domain=0:10,samples=300,>=stealth]
               ...
               \node [anchor=north] at (4,2.3){$x = A'e^{-\frac{k'}{2m}}\cos(\gamma't)+ x_0$};
...
\if0
\subsubsection{単振動+速度の2乗に比例する抗力が働く運動型}
